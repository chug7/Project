...
router.post(
  ...
  async (req, res) => {
    ...
    //Build social object
    profileFields.social = {} // must initialize first or social will be undefined causing an error
...
router.get('/', async (req, res) => {
  try {
    const profiles = await Profile.find().populate('user', ['name', 'avatar']) //we want to add the name and avatar
    res.json(profiles)
  } catch (err) {
    console.error(err.message)
    res.status(500).send('Server Error')
...
router.get('/', async (req, res) => {
  try {
    const profile = await Profile.findOne({
      user: req.params.user_id,
    }).populate('user', ['name', 'avatar']) //we want to add the name and avatar
